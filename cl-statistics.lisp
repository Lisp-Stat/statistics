...
;;; (Larry.Hunter@CUAnschutz.edu) except where otherwise noted.
  ...
  (let  ((xs (map 'list #'first points))
    ...
    (let* ((x-bar (mean xs))
           ...
           (Lxy (reduce #'+ (mapcar (lambda (xi yi) (* (- xi x-bar) (- yi y-bar)))
                                    xs ys)))
           (b (unless (zerop Lxx) (/ Lxy Lxx)))
           (a (when b (- y-bar (* b x-bar))))
           (reg-ss (* b Lxy))
           (res-ms (when (> n 2) (/ (- Lyy reg-ss) (- n 2))))
           (r (unless (or (zerop Lxx) (zerop Lxy)) (/ Lxy (sqrt (* Lxx Lyy)))))
           (r2 (unless (zerop Lyy) (/ reg-ss Lyy)))
           (t-test (unless (or (zerop Lxx) (zerop res-ms)) (/ b (sqrt (/ res-ms Lxx)))))
