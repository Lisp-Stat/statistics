...
(deftest central-moments-test1 (streaming)
  (test-moments 1000 0)
  (test-moments 1000 10)
  (test-moments 1000 100)
  (test-moments 1000 1000000))

(deftest central-moments-test2 (streaming)
...
(deftest pooled-moments-test1 (streaming)
...
(deftest test-invalid-types (streaming)
...
#|
Old test prior to big refactor
(deftest quantiles (streaming)
  (let ((sample #(0.0 1.0))
...
(deftest quantiles (streaming)
  (let ((sample #(0.0 1.0))
        ...
        (weights #(1 1))
        (expected-xs #(0.0 0.0 0.0 0.1 0.3 0.5 0.7 0.9 1.0 1.0 1.0))
        ;; 5-element sorted vector: with the 0.5 correction, q=0.5 maps
        ;; exactly to the middle element. Replaces distributions:r-normal.
        (sorted-5 #(1.0d0 2.0d0 3.0d0 4.0d0 5.0d0)))
    (assert-equality #'num= expected-xs
        (map 'vector (curry #'quantile sample) quantiles))
    (assert-equality #'num= expected-xs
        (quantiles sample quantiles))
    (assert-equality #'= 3.0d0 (quantile sorted-5 1/2))
    (assert-equality #'num= expected-xs
        (weighted-quantiles sample weights quantiles))))

(deftest quantile-probabilities (streaming)
  (let* ((n 10)
