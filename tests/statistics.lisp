...

;;; ls.statistics
(deftest interquartile-range (statistics-tests)
  (assert-equality #'= 5.5 (interquartile-range #(8 2 3 9 5 1 4 10 7 0 6))) ;vector
  (assert-equality #'= 5.5 (interquartile-range '(8 2 3 9 5 1 4 10 7 0 6))) ;list
  (assert-condition error (interquartile-range '(8 2 3 9 5 1 4 10 7 0 A))))    ;type error

(deftest fivenum (statistics-tests)
  ;; Test results obtained from https://www.icalcu.com/stat/fivenum.html
  (assert-equality #'num= #(1 2.25 4 5.75 7) (fivenum '(1 2 3 4 5 6 7)))
  (assert-equality #'num= #(1 2.5 4.5 6.5 8) (fivenum '(1 2 3 4 5 6 7 8)))
  (assert-equality #'num= #(1 2.5 4.5 6.5 8) (fivenum #(1 2 3 4 5 6 7 8)))
  (assert-equality #'num= #(2 5 8 8 15) (fivenum #(2 5 8 8 15) :tukey t))
  (assert-equality #'num= #(2 5 10 15 18) (fivenum #(2 5 8 12 15 18) :tukey t))
  (assert-condition error (fivenum '(8 2 3 9 5 1 4 10 7 0 A)))) ;type error with non-numeric value
...
(deftest test-invalid-types (statistics-tests)
  ;; If we don't muffle these, SBCL will print warnings about the very
  ;; errors we're testing for.  Although benign, it can be confusing
  ;; and appear to be a code error.
  (locally
...
(deftest quantiles (statistics-tests)
  (let ((sample #(0.0 1.0))
    ...
    (assert-equality #'= 2d0 (quantile rv 1/2))
    (assert-equality #'num= expected-xs
